...
%       q_after   = preprocess_id_data 输出的 q_s（SG 或低通后 q）
%       qd_after  = preprocess_id_data 输出的 qd_s（由所选方法得到 qd）
%       qdd_after = preprocess_id_data 输出的 qdd_s（由所选方法得到 qdd）
%       tau_after（两个口径）：
...
csv_file = '';
% true: 运行时弹窗手动选择 CSV；false: 使用 csv_file 或自动选最新 CSV
choose_csv_interactively = true;
...
% τ_id 的“补偿项”开关：
%   - 若你只想看低通效果，可以保持为空（默认等价于 tau_id ~ tau_s）
prep_opts.J_eq = [];
...
if choose_csv_interactively
    default_dir = fullfile(app_root, 'data', 'excitation');
    if ~isfolder(default_dir)
        default_dir = fullfile(app_root, 'data', '跑步');
    end
    if ~isfolder(default_dir)
        default_dir = app_root;
    end

    [file_name, file_path] = uigetfile({'*.csv', 'CSV files (*.csv)'}, ...
        '选择输入 CSV 文件', default_dir);
    if isequal(file_name, 0) || isequal(file_path, 0)
        error('plot_preprocess_before_after_trajectories: 你已取消文件选择。');
    end
...
method_name = prep_opts.derivative_method;
if isstring(method_name), method_name = char(method_name); end
...
if strcmp(method_name, 'sg')
    ...
    q_after_label = sprintf('after (q lowpass %gHz)', prep_opts.q_lowpass_fc_Hz);
    qd_after_label = 'after (q lowpass + diff)';
    qdd_after_label = 'after (q lowpass + 2nd diff)';
    title_filter_desc = sprintf('q lowpass=%gHz (ord=%d)+diff', prep_opts.q_lowpass_fc_Hz, prep_opts.q_lowpass_order);
end
